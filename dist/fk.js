 ...
 */
export function l2GripperMm(state, side) {
    const p = `${side}_arm_`;
    if (typeof state[`${p}shoulder_pan.pos`] !== "number")
        return null;
    ...
    if (typeof lift === "number")
        z -= Math.max(0, lift) / 1000;
    const mm = (m) => Math.round(m * 1000) + 0; // +0 folds Math.round's -0 to 0
    return {
        x: mm(r * Math.cos(pan)),

...
export function liftAxes(descriptor) {
...
// invent one.
export function liftJogKey(descriptor, side) {
...
export function liftKeysInState(state) {
...
export function railReading(state, key, travelMm = RAIL_TRAVEL_MM) {
    const h = state[key];
    if (typeof h !== "number")
        return { known: false, depthMm: 0, frac: 0 };
...
export function hasJointTelemetry(state) {
    return Object.keys(state).some((k) => (k.includes("_arm_") && k.endsWith(".pos")) ||
        k === "lift.pos" ||

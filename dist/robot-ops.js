 ...
 */
export function reachDofsFor(descriptor) {
...
export const ROBOT_OPS = [
    {
        ...
        agent: {
            tool: "look",
            motion: false,
            summary: "Capture a fresh still from the robot camera. Use before and after acting to verify the " +
    ...
    },
    {
        ...
        agent: {
            tool: "move_to",
            motion: true,
            summary: "Move one arm's joints to ABSOLUTE normalized targets and WAIT for arrival. Returns " +
        ...
        codegen: {
            ...
            summary: "Absolute joint move: go to a pose and HOLD it (not timed). Returns 'done' | 'blocked' | " +
    ...
    },
    {
        ...
        agent: {
            tool: "reach",
            motion: true,
            summary: "Task-space (cylindrical) jog held for ms, then stopped. Timed and open-loop — use short " +
                "pulses and re-look.",
            input_schema: {
                type: "object",
                ...
                properties: {
                    ...
                    dofs: {
                        type: "object",
                        additionalProperties: { type: "number" },
                        description: "subset of the robot's advertised task DOFs (yaw is canonical, shoulder_pan is a deprecated alias), each a rate in [-1,1]; +x forward, +y left, +z up",
                    },
                    ms: { type: "number" },
                },
            },
        ...
        codegen: {
            ...
            summary: "Task-space jog via IK. dofs: a subset of the robot's advertised task DOFs (yaw is " +
    ...
    },
    {
        ...
        codegen: {
            js: "joint",
            signature: "(side, dofs, ms)",
            summary: "Per-motor jog (no IK). dofs subset of {shoulder_pan, shoulder_lift, elbow_flex, " +
    ...
    },
    {
        ...
        agent: {
            tool: "base",
            motion: true,
            summary: "Drive the mobile base for ms. linear (+forward) and angular (+turn left), rates in " +
                "[-1,1]. Open-loop, timed.",
            input_schema: {
                type: "object",
                required: ["ms"],
    ...
    },
    {
        ...
        agent: {
            tool: "play_audio",
            motion: false,
            summary: "Play a short audio clip on the robot's speaker from a URL (CORS-enabled https:// or a " +
    ...
    // ---- codegen-only capabilities (no agent tool) -----------------------------
    {
        ...
        codegen: {
            js: "telemetry",
            signature: "()",
            summary: "-> { loopHz, safety, tempC, state:{...}, currents:{...} } or null. Proprioceptive only " +
    ...
    },
    {
        ...
        codegen: {
            js: "perceive",
            signature: "()",
            summary: "-> structured world-state from the on-Pi detector (or null if none). Lets a script react " +
    ...
    },
    {
        ...
        codegen: {
            js: "reset",
            signature: "()",
            summary: "Re-sync the IK task cursor to current joint positions. Call before a reach() that follows " +
                "any joint() move.",
        },
    },
    {
...
function jointTargetsDescription(descriptor) {
    ...
    return (`per-arm vocabulary: left arm subset of {${left.join(", ")}}; ` +
 ...
 */
export function buildAgentTools(descriptor) {
...
export function agentMotionTools() {
...
export function renderAgentToolsRef() {
...
export function renderCodegenApiRef() {
 ...
 */
export function buildRobotToolsBundle() {
    return {
...
//# sourceMappingURL=robot-ops.js.map
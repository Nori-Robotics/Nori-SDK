...
export class MockDaemonSim {
    constructor(opts) {
        ...
        this.wdState = "ok"; // last tick's watchdog verdict (edge detect)
        this.lastControlMs = null;
        this.lastTickMs = null;
        this.moved = new Set(); // joints that moved last tick (drives fake currents)
        this.navWaypoints = new Map();
        this.navActive = null;
        this.navCache = new Map();
        this.navMapSha = "a".repeat(64);
        this.sensorLidarHz = 0;
        this.sensorImuHz = 0;
        this.sensorLidarMaxPoints = 360;
        ...
        for (const j of joints)
            this.st[j] = j.includes("gripper") ? 30 : 0;
        for (const b of this.descriptor.base ?? [])
            this.st[b] = 0;
        for (const a of this.descriptor.aux ?? [])
            this.st[`${a}.pos`] = 100;
    ...
    cameraLayoutFrame() {
        ...
        if (tiles.length < 2)
            return null; // single-camera robots send no layout (matches the bridge)
        const cols = Math.ceil(Math.sqrt(tiles.length));
        return { type: "camera_layout", cols, rows: Math.ceil(tiles.length / cols), tiles: [...tiles] };
    }
    daemonStatusFrame(state = "online") {
    ...
    handleFrame(frame, nowMs) {
        ...
        const t = frame.type;
        if (t === "control")
            return this.handleControl(frame, nowMs);
        if (t === "command")
            return this.handleCommand(frame);
        if (t === "record")
            return this.handleRecord(frame);
        if (t === "navigation")
            return this.handleNavigation(frame, nowMs);
        if (t === "sensor_stream")
            return this.handleSensorStream(frame);
    ...
    rememberReply(cache, requestId, reply) {
    ...
    handleNavigation(frame, nowMs) {
        if (!this.capabilities.includes("named_navigation"))
            return [];
        const requestId = frame.request_id;
        if (!MockDaemonSim.isUuid(requestId))
            return [];
    ...
    sensorStatus(requestId, ok = true, error) {
        return {
    ...
    handleSensorStream(frame) {
        if (!this.capabilities.includes("sensor_streams"))
            return [];
        const requestId = frame.request_id;
        if (!MockDaemonSim.isUuid(requestId))
            return [];
        ...
        if (frame.action === "configure") {
            if (typeof frame.lidar_hz === "number")
                this.sensorLidarHz = frame.lidar_hz;
            if (typeof frame.imu_hz === "number")
                this.sensorImuHz = frame.imu_hz;
            ...
            result = this.sensorStatus(requestId, false, `unknown sensor stream action ${String(frame.action)}`);
        }
    ...
    sensorStamp(nowMs) {
        ...
        return { sec, nanosec: Math.floor((nowMs - sec * 1000) * 1000000) };
    }
    lidarFrame(nowMs) {
        ...
        return {
            ...
            range_max_m: 12,
            source_points: 360,
            ranges_m: Array.from({ length: points }, (_, index) => Number((2 + 0.25 * Math.sin(index * increment)).toFixed(4))),
    ...
    handleRecord(frame) {
        ...
        if (a === "episode_stop") {
            if (this.recEpisode === null)
                return [status(false, "not recording an episode")];
        ...
        if (a === "episode_discard") {
            if (!this.recSessionOpen)
                return [status(false, "no session open")];
            if (this.recEpisode !== null)
                this.recEpisode = null; // drop in-progress
            else if (this.recKept > 0)
                this.recKept -= 1; // drop just-stopped
            else
                return [status(false, "no episode to discard")];
            return [status(true)];
        }
        if (a === "session_end" || a === "stop") {
            if (!this.recSessionOpen)
                return [status(false, "no session open")];
            if (a === "stop" && this.recEpisode !== null) {
                this.recEpisode = null;
                this.recKept += 1;
            }
            this.recSessionOpen = false;
    ...
    handleControl(frame, nowMs) {
        this.lastControlMs = nowMs;
        const out = [];
        if (frame.jog && typeof frame.jog === "object")
            this.jog = frame.jog;
        if (frame.reset && typeof frame.reset === "object") {
            for (const [armKey, on] of Object.entries(frame.reset)) {
                if (!on)
                    continue;
        ...
        if (frame.action && typeof frame.action === "object") {
            ...
            if (this.safety === "latched") {
                ...
                if (id)
                    out.push(this.actionStatus(id, "blocked", "estop_latched"));
                return out;
            }
            ...
            const unknown = [];
            let clamped = false;
            for (const [key, v] of Object.entries(frame.action)) {
            ...
            if (id && !Object.keys(targets).length) {
                out.push(this.actionStatus(id, "blocked", unknown.length ? "unknown_joint:" + unknown.sort().join(",") : "empty_action"));
                return out;
            }
            this.pending.push({ id, targets, clamped, announcedActive: false });
            if (id)
                out.push(this.actionStatus(id, "accepted"));
        }
        return out;
    }
    ...
    handlePose(pose, frame) {
        if (!this.capabilities.includes("pose_targets"))
            return []; // unadvertised: dropped in silence
        const id = typeof frame.action_id === "string" ? frame.action_id : "";
        const refuse = (reason) => (id ? [this.actionStatus(id, "blocked", reason)] : []);
        if (this.safety === "latched")
            return refuse("estop_latched");
        // Sides = the arms this robot HAS that the payload addresses (gateway: self.arms).
        const armOf = (k) => k.slice(0, -"_arm".length);
        const hasArm = (side) => (this.descriptor.joints ?? []).some((j) => j.startsWith(`${side}_arm_`));
        const sides = Object.keys(pose).filter((k) => k.endsWith("_arm") && hasArm(armOf(k))).map(armOf);
        if (!sides.length)
            return refuse("empty_pose");
        if (sides.length > 1)
            return refuse("one_arm_per_pose");
        const side = sides[0];
        const target = pose[`${side}_arm`];
        if (!target || typeof target !== "object")
            return refuse("bad_pose");
        const frameName = String(target.frame ?? "");
        if (frameName !== "base_footprint")
            return refuse(`frame:${frameName || "missing"}`);
        ...
        if (orientation !== undefined && (!Array.isArray(orientation) || orientation.length !== 4
            || !orientation.every((v) => typeof v === "number")))
            return refuse("bad_pose");
    ...
    handleCommand(frame) {
        ...
        const name = typeof frame.name === "string"
            ? frame.name
    ...
    tick(nowMs) {
        ...
        if (this.lastControlMs !== null) {
            ...
            if (silence > this.watchdog.t_stop_ms)
                wd = "stop";
            else if (silence > this.watchdog.t_warn_ms)
                wd = "warn";
        }
        if (wd === "stop" && this.wdState !== "stop") {
            ...
            for (const p of this.pending) {
                if (p.id)
                    out.push(this.actionStatus(p.id, "timeout", "watchdog_stop"));
            }
            this.pending = [];
        }
        this.wdState = wd;
    ...
    integrateJog(dt) {
        ...
        for (const side of ["left_arm", "right_arm"]) {
            ...
            if (!arm || typeof arm !== "object")
                continue;
            for (const [dof, rateU] of Object.entries(arm)) {
        ...
        if ("x.vel" in this.st)
            this.st["x.vel"] = typeof b.linear === "number" ? b.linear : 0;
        if ("theta.vel" in this.st)
            this.st["theta.vel"] = typeof b.angular === "number" ? b.angular : 0;
        if (this.st["x.vel"] || this.st["theta.vel"])
            this.moved.add("base");
    ...
    slewActions(dt) {
        ...
        this.pending = this.pending.filter((p) => {
            ...
            if (reached && p.id)
                out.push(this.actionStatus(p.id, p.clamped ? "clamped" : "done"));
            return !reached;
        });
        return out;
    }
    telemetryFrame(nowMs, wd) {
        var _a;
        const currents = {};
        ...
        // as a real motor.
        if (this.idleCurrentMotor)
            currents[_a = this.idleCurrentMotor] ?? (currents[_a] = 20 + Math.floor(this.noise() * 20));
        return {
            type: "telemetry",
    ...
    // ---- helpers ---------------------------------------------------------------------------
    actionStatus(id, state, reason) {
        const f = { type: "action_status", action_id: id, state, ts_ns: this.lastTickMs === null ? 0 : Math.round(this.lastTickMs * 1e6) };
        if (reason)
            f.reason = reason;
        return f;
    }
...
MockDaemonSim.REQUEST_HISTORY = 256;

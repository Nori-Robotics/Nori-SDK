...
export function createMockRobot(opts) {
    ...
    const sessionBusy = () => (dc !== null && dc.readyState === "open") ||
    ...
    const drawComposite = (nowMs) => {
        ...
        const tiles = layout ? layout.tiles : (sim.descriptor.cameras ?? ["front"]);
    ...
    const buildSession = async () => {
        ...
        channel.onopen = () => {
            ...
            tickTimer = setInterval(() => {
                for (const f of sim.tick(performance.now()))
                    dcSend(f);
            }, telemetryMs);
            ...
            if (canvas)
                drawTimer = setInterval(() => drawComposite(performance.now()), 1000 / DRAW_FPS);
        };
        channel.onmessage = (ev) => {
            ...
            for (const f of sim.handleFrame(frame, performance.now()))
                dcSend(f);
        };
        ...
        if (gen !== generation || !pc)
            return; // torn down while negotiating
    ...
    const announce = (rateLimited = false) => {
        ...
        if (rateLimited && now - lastAnnounceMs < HANDSHAKE_RATE_LIMIT_MS)
            return;
        lastAnnounceMs = now;
        port.announce({ nonce });
    };
    ...
    const nack = (reason) => {
        const now = performance.now();
        if (now - lastNackMs < HANDSHAKE_RATE_LIMIT_MS)
            return;
        lastNackMs = now;
    ...
    port.onSdp((p) => {
        ...
        void peer
            ...
            for (const c of queued)
                void peer.addIceCandidate(c).catch(() => { });
        })
            .catch((e) => log("mock robot: answer failed: " + e.message));
    });
    port.onIce((p) => {
        if (!pc)
            return;
